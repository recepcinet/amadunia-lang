...
| [proposal-frequency.md](proposal-frequency.md) | Where *daima* and *kadang* stand. Thirteen uses in three positions; *tena* settled itself 32 of 32. **Open — the founder's call.** |
...
alone. The row for ["want to be" plus an adjective](proposal-modal-adjective.md)
called it *the smallest of the four* — a judgement its own briefing opens by
withdrawing, in the sentence **"Measured, it is not small."** The same row was
also the only open briefing in the table with no **Open — the founder's call.**
...
- **A noun with an adjective is now both a sentence and a phrase.** *Dom merah* is "the house is red" by the copula rule, and since [the fragment rule](sentence-types.md) of September 3, 2026 it is also "the red house" standing alone. 154 two-word utterances in the repository have this shape. Found September 5, 2026 by reading Lesson 12 end to end: its table glosses *Dom merah.* one way and its conversation glossed the same string the other way, eighty lines apart. Recorded, not briefed — context has carried it everywhere else.
...
- **Whether a time word may open a sentence.** The order puts time at the end and grants fronting to a subordinate clause alone, and eight sentences front a time word instead — *sasa ta hao*, *Besok mi saufa go dom mi* — two of them in lessons. Every one is a contrast, which is what fronting is for in the languages this one borrows from. **Open — the founder's call.**
