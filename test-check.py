...
MUTATIONS = [
    ...
    # The expectations below name the fault, not the corpus figure in the
    # message: a mutation that has to be re-typed every time a text is written
    # is a second copy of the count, and six of these went NOT APPLIED or
    # WRONG CHECK together on September 6, 2026 for no better reason.
    ("the front page's text count gone stale", "README.md",
     "[texts/](texts/) — thirty original pieces |",
     "[texts/](texts/) — twenty original pieces |",
     "says 'twenty original pieces'; texts/ holds"),
    ...
    ("a text count outside texts/ gone stale", "dictionary/frequency.md",
     "thirty texts and the phrasebook", "twenty texts and the phrasebook",
     "says 'the twenty texts'; texts/ holds"),
    ...
    ("the gap list's writing count gone stale", "dictionary/README.md",
     "fourteen name a text or a lesson", "twelve name a text or a lesson",
     "rows name a text or a lesson, and the page"),
    ("the gap list's question count gone stale", "dictionary/README.md",
     "fifteenth and sixteenth came from neither",
     "fifteenth came from neither",
     "gaps found by a question are not"),
    ("the gap list's feelings count gone stale", "dictionary/README.md",
     "Three of the sixteen are about how a person feels",
    ...
    ("a briefing's corpus row gone stale", "grammar/proposal-modal-adjective.md",
     "| Amadunia sentences in the material | 1895 |",
    ...
    ("the both-ways count mistaken for the table", "grammar/definiteness.md",
     "**Forty-one nouns** are translated both ways", "**Seven nouns** are translated both ways",
     "says Seven nouns are translated both ways; the material gives 41"),
    ...
    ("a rejection table naming fewer collisions than exist",
     "grammar/place.md", "minimal pair with *go*, *ke*, *no* |",
     "minimal pair with *go* |",
     "ko is a minimal pair with go, ke, no, and the table says go"),
    ("a candidate rejected for containing a word it does not contain",
     "grammar/questions.md", "| kapan (Indonesian) | when | ends in *pan* (bread) |",
     "| kapan (Indonesian) | when | ends in *kim* (who) |",
     "rejects kapan for containing kim, which is not in it"),
    ("a rejection reason naming the wrong collision",
     "grammar/conjunction.md", "minimal pair with *kim* |",
     "minimal pair with *kan* |",
     "kum is a minimal pair with kim, and the table says kan"),
    ("the count of roots ending in -o gone stale",
     "grammar/conjunction.md", "and 13 roots end in *-o*",
     "and four roots end in *-o*", "13 roots end in -o"),
    ...
    ("a second copy of a rule detector", "check.py",
     ...
     '        _r.add("una"); _r.add("una")',
     "detects the una rule in more than one place"),
    ("the tracked-rule count gone stale", "texts/README.md",
     "**Eighteen rules can be spotted in a sentence**",
     "**Seventeen rules can be spotted in a sentence**",
     "the scan tracks 18 rules"),
    ("the rule-page count gone stale", "texts/README.md",
     "There are twenty-one rule pages", "There are twenty rule pages",
     "grammar/ holds 21 rule pages"),
    ("the question rule dropped from the exercise table", "texts/README.md",
     "| question | 12 |\n", "", "rule table has drifted from the scan"),
    ("the missing count spelled out gone stale", "dictionary/proposal-a2.md",
     "A hundred and six are not.", "A hundred and five are not.",
     "106 concepts are missing and the page must say so in words"),
    ("a source language with no family", "dictionary/dictionary.md",
     "Thai *nakhon*", "Tagalog *nakhon*, Vietnamese",
     "names a language the family map does not know"),
    ("a family row dropped from the balance table", "dictionary/balance.md",
     "| Celtic | 0 | 0.0% | 1 | 0.3% |\n", "",
     "family table has drifted from the dictionary"),
    ("a sort key with nothing to break its ties", "check.py",
     "_rank = sorted(_reach.items(), key=lambda kv: (-kv[1], kv[0]))",
     "_rank = sorted(_reach.items(), key=lambda kv: -kv[1])",
     "sorts with a single-term key"),
    ...
    ("a root doing two jobs outside the two that are named",
     ...
     # check to see it, and mutating only the gloss row proved nothing.
     "texts/text-21-uan-umur.md", "Ta-ta kanta tena.", "Ta-ta kanta kanta.",
     "a verb root stands twice in a row"),
    ("the open fronted-time shape spreading", "texts/text-20-in-skola.md",
     "Sasa mi faham", "Sasa mi faham. Besok ta lai",
     "sentences open with a time word"),
    ("a fronted-time sentence not named on the rule page", "grammar/place.md",
     "| *Sasa mi faham* | now I understand |",
     "| *Sasa mi bil* | now I know |",
     "must name every sentence that fronts a time word"),
    ("a gloss borrowing the perfect", "texts/text-6-seti-din.md",
    ...
    ("the open tense-on-adjective shape spreading", "texts/text-16-berapa-harga.md",
     "Mi fikir samaki saufa hao", "Mi fikir samaki saufa hao. Ca suda barid",
     "sentences put a tense marker in front of an adjective"),
    ("a sentence using the open shape not named on the rule page",
     "grammar/tense.md", "| *Ta suda bimar* | she was sick |",
     "| *Ta suda mal* | she was bad |",
     "must name every sentence that uses the open shape"),
    ("a composed concept dropped from the checklist's note",
     ...
     "must name the concepts the material composes"),
    ("the kinship block's own size gone stale",
     "dictionary/a1-checklist.md", "**Thirteen of the missing are kinship",
     "**Twelve of the missing are kinship",
     "13 of the people domain are missing from the index, not thirteen"),
    ("the kinship split not accounting for the block",
     "dictionary/a1-checklist.md", "**Six are not gaps at\nall**",
     "**Five are not gaps at\nall**",
     "the three parts of the kinship block are [3, 5, 4]"),
    ...
    ("a rejection overstating a candidate's syllables", "grammar/negation.md",
     ...
     "says tidak has 3 syllables; it has 2"),
    ("a rejection miscounting a candidate's letters",
     "grammar/proposal-but.md", "| tetapi | the formal Indonesian form | passes; six letters",
     "| tetapi | the formal Indonesian form | passes; five letters",
     "says tetapi has 5 letters; it has 6"),
    ("an ending count gone stale on a rule page", "grammar/place.md",
     "| a | to (Spanish) | 69 roots end in *-a*",
     "| a | to (Spanish) | thirty roots end in *-a*",
     "says 30 roots end in -a; 69 do"),
    ("an ending count gone stale on a second page", "grammar/verb-chains.md",
     "| a | Spanish | 69 roots end in *-a*",
     "| a | Spanish | thirty roots end in *-a*",
     "says 30 roots end in -a; 69 do"),
    ("the prose-residue count gone stale in the briefing",
     "grammar/proposal-sentence-types.md", "**21 runs come from",
     "**20 runs come from",
     "21 runs come from explanatory prose"),
    ("the prose-residue count gone stale in the texts index",
     "texts/README.md", "**21 runs, 55 words**", "**20 runs, 55 words**",
     "21 runs come from explanatory prose"),
    ...
    ("a lesson naming something that is not a command",
     ...
     "*Mi mau go*) and three",
     "names 4 commands and each must begin with a verb"),
    ("a lesson's conversation sentence count gone stale",
     "lessons/lesson-26-telling-and-answering.md",
     "Seven of those eleven sentences", "Seven of those twelve sentences",
     "conversation has 11 sentences"),
    ("the strict count of one-word turns gone stale",
     "lessons/lesson-26-telling-and-answering.md",
     "are two, *Ya* and *No*, and the total is six",
     "are three, *Ya* and *No*, and the total is six",
     "counted strictly there are 2 one-word turns"),
    ("a text's summary heading the wrong sentence count",
     "texts/text-15-sol-lai.md", "Twenty-seven sentences and 42 roots",
     "Twenty-three sentences and 42 roots",
     "heads its summary 'Twenty-three sentences and 42 roots'"),
    ("a rule's uses counted against the wrong sentence total",
     "texts/text-11-anak-katab.md", "eleven times in seventeen sentences",
     "eleven times in eighteen sentences",
     "says 'times in eighteen sentences'"),
    ("text 21's shape count gone stale", "texts/text-21-uan-umur.md",
     "uses **52 distinct shapes**", "uses **56 distinct shapes**",
     "text-21 uses 52 distinct shapes"),
    ("the runner-up shape count gone stale", "texts/text-21-uan-umur.md",
     "with 27 each", "with 24 each",
     "the next highest shape count is 27"),
    ("text 21's concentration figure gone stale",
     "texts/text-21-uan-umur.md", "cover 33% of it", "cover 31% of it",
     "its three commonest shapes cover 33%"),
    ("text 21's least-repetitive rank gone stale",
     "texts/text-21-uan-umur.md", "the eighth *least*", "the fifth *least*",
     "text-21 is the 8th least repetitive of"),
    ("text 21's adjacent-pair rank gone stale in the index",
     "texts/README.md", "puts it fifteenth of thirty",
     "puts it fifteenth of twenty-one",
     "text-21 shares a shape between"),
    ("the vowel-ending share gone stale in the poem's page",
     "texts/text-5-uan.md", "Half the three hundred roots end in a vowel",
     "A tenth of the three hundred roots end in a vowel",
     "roots end in a vowel"),
    ("the -a ending share gone stale", "texts/text-5-uan.md",
     "23% end in\n*-a* alone", "31% end in\n*-a* alone",
     "23% of roots end in -a"),
    ("the -ar and -an share gone stale", "texts/text-5-uan.md",
     "another 8% between them", "another 12% between them",
     "-ar and -an are 8% between them"),
    ("a fraction rounded up in the poem's page", "texts/text-5-uan.md",
     "where nearly a quarter of all words", "where a quarter of all words",
     "the commonest ending is 23%"),
    ("the ladder losing its Lesson 01 row", "lessons/reading-ladder.md",
     "| 01 | 23% |\n", "", "reading-ladder.md has no row for Lesson 01"),
    ("the ladder's opening figure gone stale in its own prose",
     "lessons/reading-ladder.md", "after one lesson — 23%.**",
     "after one lesson — 22%.**",
     "says 22% of the texts' words after one lesson"),
    ("a fraction rounded up on the front page", "README.md",
     "— nearly a quarter of the texts' words", "— a quarter of the texts' words",
     "calls 23% a quarter"),
    ("an open briefing left unmarked in the grammar index",
     "grammar/README.md",
     "and not one place the two may meet. **Open — the founder's call.** |",
     "and not one place the two may meet. |",
     "says Six briefings are open and marks 5 rows"),
    ("a decided briefing marked open in the grammar index",
     "grammar/README.md",
     "the rules are [sentence-types.md](sentence-types.md). |",
     "the rules are [sentence-types.md](sentence-types.md). **Open — the founder's call.** |",
     "row for proposal-sentence-types.md is marked open though the briefing is decided"),
    ("the tena figure gone stale in the grammar index", "grammar/README.md",
     "*tena* settled itself 32 of 32", "*tena* settled itself 29 of 29",
    ...
    ("an action held as a noun dropped from the checklist's note",
     ...
     "| **rabota**, the action |",
     "are missing as actions and held as nouns"),
    ("a quality held as a noun dropped from the checklist's note",
     "dictionary/a1-checklist.md", "| **angry** | *gusa*, *anger* |",
     "| **sad** | *sedih* |", "angry is missing and anger is held"),
    ("the checklist's deliberate duplicate undocumented",
     "dictionary/a1-checklist.md", "*clean* is in **actions**",
     "*safi* is in **actions**",
     "1 concepts appear twice — clean — and one deliberate duplicate"),
    ("the phrasebook's Lesson 01 line count gone stale", "phrasebook.md",
     "gets you five of the eight lines", "gets you six of the eight lines",
     "5 of the first section's lines need nothing past Lesson 01"),
    ("a line Lesson 01 covers dropped from the phrasebook's list",
     "phrasebook.md", "*Ya, ok.* and *Bai!*", "and *Bai!*",
     "does not name the lines Lesson 01 covers"),
    ("the phrasebook understating when its first section is complete",
     "phrasebook.md", "waits\nfor **Lesson 16**", "waits\nfor **Lesson 03**",
     "the first section is not complete until Lesson 16"),
    ("the phrasebook's last word placed in the wrong lesson", "phrasebook.md",
     "arrives in\n[Lesson 25]", "arrives in\n[Lesson 24]",
     "its last word arrives in Lesson 25"),
    ...
    ("the six-family count gone stale", "dictionary/balance.md",
     "Fourteen more name six", "Twelve more name six",
     "14 roots name exactly six families"),
    ...
    ("the gap list's three parts not adding up", "dictionary/README.md",
     "**Seven of the sixteen are joints", "**Eight of the sixteen are joints",
     "the three parts of the gap list are"),
    ("the unmarked-clause count gone stale", "grammar/subordination.md",
     "**11 without one**", "**9 without one**",
     "subordination.md's two counts are stale"),
    ("a vowel sequence illustrated with a word that lacks it",
     "grammar/phonology.md", "| ua | *uan*, *akua* |", "| ua | *uan*, *una* |",
     "illustrates the sequence 'ua' with una"),
    ("a vowel sequence row dropped from phonology.md",
     "grammar/phonology.md", "| ao | *hao* |\n", "",
     "gives a row to 4 vowel sequences; 5 are attested"),
    ("a consonant pair illustrated with a root that lacks it",
     "grammar/phonology.md", "- `mp` — *tempo*", "- `mp` — *dunia*",
     "illustrates the consonant pair 'mp' with the root dunia"),
    ("text 8's yu-sentence count gone stale",
     "grammar/proposal-sentence-types.md", "Text 8 has **12** sentences",
     "Text 8 has **10** sentences",
     "text 8 opens 12 sentences with yu and a verb"),
    ("the example block and the sentence introducing it disagreeing",
     "grammar/proposal-sentence-types.md", "the sentence says\nfive now",
     "the sentence says\nseven now",
     "prints 5 example commands and does not say so"),
    ("a command in the block that is not in the list of eight",
     "grammar/proposal-sentence-types.md", "Go yamin.            Go right.",
     "Go dom.              Go home.",
     "the block prints a command missing from its own list of eight"),
    ("a missing command quietly dropped from the record",
     "grammar/proposal-sentence-types.md", "*Anda yamin*, *Funga",
     "*Funga", "of the eight are absent from the block"),
    ("the three-syllable list no longer parseable",
     "grammar/proposal-stress.md", "three-syllable roots — *bahari",
     "three-syllable roots: *bahari",
     "no longer lists the three-syllable roots"),
    ("a two-syllable root listed among the three-syllable ones",
     "grammar/proposal-stress.md", "kemarin, madina", "kucing, madina",
     "lists kucing among the three-syllable roots"),
    ("the unnamed remainder of the three-syllable roots miscounted",
     "grammar/proposal-stress.md", "and the other eighteen",
     "and the other seventeen",
     "names 16 of the 34 three-syllable roots, so 18 are unnamed"),
    ("the vowel-pair root count gone stale", "grammar/proposal-stress.md",
     "In 28 of the 300 roots", "In 27 of the 300 roots",
     "28 roots contain an attested vowel pair"),
    ("a root named as containing a vowel pair that does not",
     "grammar/proposal-stress.md", "naiti, nau", "nikah, nau",
     "names roots as containing a vowel pair that do not"),
    ("the unnamed remainder of the vowel-pair roots miscounted",
     "grammar/proposal-stress.md", "and twelve others", "and eleven others",
     "names 16 of the 28 affected roots, so 12 are unnamed"),
    ("what the final-versus-penultimate choice separates",
     "grammar/proposal-stress.md", "separates 243 of the 300 roots",
     "separates 100 of the 300 roots",
     "penultimate and final differ on every root of two syllables or more"),
    ("a letter illustrated with a word that is not a root",
     "grammar/pronunciation.md", "| *garam* |", "| *garamu* |",
     "illustrates a letter with a word that is not a root"),
    ("a letter illustrated with a word that does not contain it",
     "grammar/pronunciation.md", "| **p** | /p/ | | *pan* |",
     "| **p** | /p/ | | *dom* |",
     "illustrates a letter with a word that does not contain it"),
    ("a letter dropped from the pronunciation tables",
     "grammar/pronunciation.md", "| **t** | /t/ | | *ta* |", "",
     "gives a row to 19 letters; the alphabet has 20"),
    ("the reduction rule naming the wrong condition",
     "grammar/pronunciation.md", "no reduced variant away",
     "no reduced variant under",
     "a vowel reduces when it is unstressed"),
    ("the re-stressed share gone stale", "grammar/stress.md",
     "eleven per cent of the dictionary", "twelve per cent of the dictionary",
     "share figure is stale; 34 of 300 is 11 per cent"),
    ("the cost ratio rounded the flattering way", "grammar/stress.md",
     "thirty-four is 2.56,", "thirty-four is 2.99,",
     "ratio is stale; 87 to 34 is 2.56"),
    ("the three-syllable figure gone stale", "grammar/stress.md",
     "go wrong on the 34 three-syllable roots",
     "go wrong on the 33 three-syllable roots",
     "the dictionary has 34 roots of three syllables"),
    ("a root dropped from the shared list of the two thirty-fours",
     "grammar/stress.md", "*restoran*, *telefon*", "*restoran*",
     "the two sets of 34 share 4 roots"),
    ("the two-syllable half of the moved roots miscounted",
     "grammar/stress.md",
     "**Thirty of the roots whose", "**Twenty of the roots whose",
     "30 of the moved roots have two syllables"),
    ("the number-word count gone stale on the front page", "README.md",
     "twelve number words reach a thousand",
     "eleven number words reach a thousand",
     "the number system is 12 words, eleven of which reach a hundred"),
    ("the status line not naming the largest settled number",
     "grammar/numbers.md",
     "*Status: settled for 1\u201310, 100, 1000.*",
     "*Status: settled for 1\u201310, 100.*",
     "the largest settled number is 1000 and the status line does not name it"),
    ("the built-number exclusion naming the wrong total",
     "dictionary/dictionary.md", "are not counted in the 300",
     "are not counted in the 113",
     "the count the built numbers are excluded from is not 300"),
    ("the existential count gone stale", "grammar/sentence-types.md",
     "standing in **44** sentences", "standing in **30** sentences",
     "existential count is stale; a sentence-initial es stands in 44"),
    ...
    ("the exercise floor gone stale", "texts/README.md",
     "each stand in **at least eight** texts", "each stand in **at least five** texts",
     "claims a floor of"),
    ...
    ("a text's count of the pages without *mi* gone stale",
    ...
    ("a text left out of the index", "texts/README.md",
     "| [Din mi](text-28-din-mi.md) |", "| Din mi |",
     "index does not list"),
    ("a ladder row that ignores the grammar", "lessons/reading-ladder.md",
     "| [Din mi](../texts/text-28-din-mi.md) | 06 |",
     "| [Din mi](../texts/text-28-din-mi.md) | 04 |",
     "contradicts the row"),
    ("the index's copy of the adjective-predicate figure gone stale",
     "grammar/README.md",
     "405 verbless adjective predicates", "377 verbless adjective predicates",
     "verbless adjective predicates; the material gives"),
    ("the index's copy of the modal figure gone stale", "grammar/README.md",
     "59 working modals", "60 working modals",
     "working modals; the material gives"),
    ("a rule the ladder dates and no scan can find", "lessons/README.md",
     "| number | 01 |", "| number | 01 |\n| ellipsis | 01 |",
     "dated but never found"),
    ("a question word read as a clause marker", "check.py",
     'and not ("?" in _line and _t[-1] in ("porke", "kab"))',
     'and not ("?" in _line and _t[-1] in ("porke", "kabx"))',
     "contradicts the row"),
    ("the question split miscounted", "texts/README.md",
     "Of the material's 259 questions, 164 use no",
     "Of the material's 259 questions, 160 use no",
     "of them use no question word"),
    ("the tone question dated with the question word", "texts/README.md",
     "**Eighteen rules can be spotted", "**Seventeen rules can be spotted",
     "the scan tracks 18 rules"),
    ("a lesson using a rule dated after itself", "lessons/lesson-10-a-day.md",
     "| Lai sini. | Come here. |", "| Mi no go. | I do not go. |",
     "rule, which Lesson 14 introduces"),
    ("una read as the preposition when it is the adverb",
     "lessons/lesson-02-food-and-home.md",
     "Kita kula una.", "Kita kula una pan.",
     "uses the una rule"),
    ("the prose-run word count gone stale", "texts/README.md",
     "**21 runs, 55 words**", "**21 runs, 51 words**",
     "runs come from explanatory prose"),
    # The date scan flattens the file first: five dates in the future survived
    # it for two days by wrapping between the day and the year, so the
    # replacement here wraps on purpose. Assembled from pieces so this file is
    # not itself dated in the future.
    ("a future date split across a line break", "dictionary/balance.md",
     "from September 6,\n2026", "from September 30," + "\n2027",
     "which has not happened"),
    ("a candidate row that drops out of the table", "dictionary/balance.md",
     "| 学校 *gakkō*, school | *gako* | passes |",
     "| 学校 *gakkō*, school | gako | passes |",
     "the paragraph above it counts"),
    ("a candidate borrowing with a hand-written verdict",
     "dictionary/balance.md",
     "| 元気 *genki*, well, lively | *genki* | passes |",
     "| 元気 *genki*, well, lively | *genki* | rejected |",
     "balance.md says genki rejected; the rules say it passes"),
    ("the candidate pass count gone stale", "dictionary/balance.md",
     "and seven passed:", "and eight passed:",
     "of the ten candidates pass every rule"),
    ("the widest root's families misnamed", "dictionary/balance.md",
     "belongs to seven: Austronesian, Indo-Aryan",
     "belongs to seven: Austronesian, Iranian",
     "does not name sabun's families"),
    ("the Korean-reaching roots misnamed", "dictionary/balance.md",
     "*ok* and *pan* name it among their sources",
     "*ok* and *ca* name it among their sources",
     "the roots that name a Korean source are"),
    ("a rule dated later than the lesson that uses it", "lessons/README.md",
     "| copula | 06 |", "| copula | 11 |",
     "uses the copula rule, which Lesson 11 introduces"),
    ("a link labelled with the wrong number", "dictionary/README.md",
     "| pain, to hurt | [text 6](../texts/text-6-seti-din.md)",
     "| pain, to hurt | [text 7](../texts/text-6-seti-din.md)",
     "a link labelled '7' points at text-6-seti-din.md"),
    ("the front page's root count in passing", "README.md",
     "teaching every one of the 300 roots", "teaching every one of the 299 roots",
     "the dictionary has 300"),
    ("the front page's lesson count in passing", "README.md",
     "| 26 lessons in order", "| 25 lessons in order",
     "lessons/ holds 26"),
    ("the verb-chain lesson named in passing", "README.md",
     "chain two verbs before Lesson 17", "chain two verbs before Lesson 16",
     "the verb chain arrives in Lesson 17"),
    ("the front page's arithmetic", "README.md",
     "21 = *du-des-uan*", "21 = *du-des-du*",
     "that spelling is 22"),
    ("a number root glossed against its own digit", "README.md",
     "*ba* (8) is Chinese", "*ba* (9) is Chinese",
     "the dictionary glosses ba as 8"),
    ("the count of roots glossed as verbs", "README.md",
     "19 of the 47 roots the dictionary glosses",
     "19 of the 46 roots the dictionary glosses",
     "roots are glossed that way"),
    ("the two verb counts confused again", "README.md",
     "The language has 50 verbs", "The language has 47 verbs",
     "words behave as verbs"),
    ("a fraction named with a one-digit lesson", "lessons/README.md",
     "half by Lesson 06, and", "half by Lesson 6, and",
     "the ladder first reaches 50% at Lesson 06"),
    ("story 1's rung gone stale in the lesson index", "lessons/README.md",
     "becomes readable at Lesson 13", "becomes readable at Lesson 14",
     "the ladder opens it at Lesson 13"),
    ("a gap row that lost a cell wall", "dictionary/README.md",
     "| a rule | [text 27](../texts/text-27-kalima-por-anak-anak.md) |",
     "| a rule | [text 27](../texts/text-27-kalima-por-anak-anak.md) | | ",
     "cells against the"),
    ("a text's command count gone stale", "texts/text-27-kalima-por-anak-anak.md",
     "**Fifteen of the twenty-one sentences are commands**",
     "**Fourteen of the twenty-one sentences are commands**",
     "of its sentences are commands"),
    ("a text's negative-command count gone stale",
     "texts/text-27-kalima-por-anak-anak.md",
     "and three of those are\nnegative", "and five of those are\nnegative",
     "of those negative; the text has"),
    ("a text whose stops and sentences disagree",
     "texts/text-27-kalima-por-anak-anak.md",
     "Buka libro yu. Kara.", "Buka libro yu. Kara..",
     "the two readings disagree"),
    ("the most repeated sentence gone stale", "texts/README.md",
     "appears nine times across six files", "appears ten times across six files",
     "most-repeated sentence is stale"),
    ...
    ("the rule table loses a row", "lessons/README.md",
     "| command | 10 |", "",
     "found but never dated"),
    ...
    ("a command before Lesson 10", "lessons/lesson-08-getting-by.md",
     ...
     "| Mi rabota sini. | I work here. |\n| Kula pan. | Eat bread. |",
     "uses the command rule, which Lesson 10 introduces"),
